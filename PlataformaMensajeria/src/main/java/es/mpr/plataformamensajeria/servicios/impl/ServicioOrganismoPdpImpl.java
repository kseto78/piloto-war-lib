 ...
 * 
 */
@Service("servicioOrganismoPdpImpl")
public class ServicioOrganismoPdpImpl implements ServicioOrganismoPdp {
	...
	private static Logger logger = Logger
			.getLogger(ServicioOrganismoPdpImpl.class);

	/**  tbl organismos manager. */
	@Resource(name = "TblOrganismosPdpManagerImpl")
	private TblOrganismosPdpManager tblOrganismosPdpManager;
	...
	@Resource(name = "queryExecutorOrganismosPdpImpl")
	private QueryExecutorOrganismosPdp queryExecutorOrganismosPdpImpl;
	...
	// //MIGRADO
	@Override
	public List<OrganismoPdpBean> getOrganismosPdp() throws BusinessException {
		try {
			TblOrganismosPdpQuery query = new TblOrganismosPdpQuery();
			query.setEliminadoIsNull(true);
			query.addOrder("nombre", OrderType.ASC);
			List<TblOrganismosPdp> lista = tblOrganismosPdpManager
					.getOrganismosPdpByQuery(query);

			return getListOrganismoPdpBean(lista);
		} catch (Exception e) {
			logger.error("ServicioOrganismoImpl - getOrganismos:" + e);
			throw new BusinessException(e,
					ServicioOrganismoPdpImpl.ERRORS_ORGANISMO_GET_ORGANISMOSPDP);
		}
	}
	...
	// /MIGRADO
	@Override
	public List<Integer> getOrganismosPdp(String rolUsuario, Integer idUsuario)
	...
	// //MIGRADO
	@Override
	public PaginatedList<OrganismoPdpBean> getOrganismosPdp(int start, int size,
			String order, String columnSort, OrganismoPdpBean criterio)
			throws BusinessException {
		...
		try {
			...
			es.minhap.plataformamensajeria.iop.beans.OrganismoPdpBean ob = new es.minhap.plataformamensajeria.iop.beans.OrganismoPdpBean();
			ob = createOrganismoPdpBean(criterio, ob);
			List<TblOrganismosPdp> lista = queryExecutorOrganismosPdpImpl
					.getOrganismosPdpPaginado(start, size, order, column, ob);

			List<OrganismoPdpBean> pageList = getListOrganismoPdpBean(lista);

			// Total de organismos
			Integer rowcount = queryExecutorOrganismosPdpImpl
					.countOrganismosPdpPaginado(ob);

			PaginatedList<OrganismoPdpBean> result = new PaginatedList<>();
	...
	@Override
	@Transactional
	public Integer newOrganismoPdp(OrganismoPdpBean organismoPdp, String source,
			String accion, Long accionId) throws BusinessException {
		try {
			TblOrganismosPdp organismoTO = getOrganismoPdpTO(organismoPdp);
			...
			Long idOrganismo = tblOrganismosPdpManager.insert(organismoTO, source,
					accion, accionId);		
			organismoPdp.setFechacreacion(organismoTO.getFechacreacion());
			organismoPdp.setCreadopor(organismoTO.getCreadopor());
			organismoPdp.setOrganismoPdpId(idOrganismo.intValue());
			return organismoPdp.getOrganismoPdpId();
		} catch (Exception e) {
	...
	public void updateOrganismoPdp(OrganismoPdpBean organismoPdp, String source, String accion, Long accionId) throws BusinessException {
		
		try {
			TblOrganismosPdp organismoPdpTO = getOrganismoPdpTO(organismoPdp);
			...
			tblOrganismosPdpManager.update(organismoPdpTO, source, accion, accionId);
	...
	@Override
	@Transactional
	public OrganismoPdpBean loadOrganismoPdp(OrganismoPdpBean organismoPdp)
			throws BusinessException {
		try {
			TblOrganismosPdp organismoPdpTO = tblOrganismosPdpManager.getOrganismoPdpById(organismoPdp.getOrganismoPdpId().longValue());
			return getOrganismoPdpBean(organismoPdpTO);
		} catch (Exception e) {
			logger.error("ServicioOrganismoPdpImpl - loadOrganismoPdp:" + e);
	...
	public void deleteOrganismoPdp(Long organismoPdpId, String source, String accion,
		...
		try {
			TblOrganismosPdp o = tblOrganismosPdpManager
					.getOrganismoPdpById(organismoPdpId);

			o.setModificadopor(PlataformaMensajeriaUtil.getUsuarioLogueado()
					.getNombreCompleto());
			o.setFechamodificacion(new Date());
			o.setEliminado("S");			
			tblOrganismosPdpManager.update(o, source, accion, accionId);
		} catch (Exception e) {
	 ...
	 */
	@Override
	public Boolean existeOrganimo(OrganismoPdpBean organismoPdp) {
		try {
			TblOrganismosPdpQuery query = new TblOrganismosPdpQuery();
	 ...
	 */
	// ///MIGRADO
	protected TblOrganismosPdp getOrganismoPdpTO(OrganismoPdpBean organismoPdp) {

		TblOrganismosPdp organismoPdpTO = new TblOrganismosPdp();
		...
		try {
			...
			organismoPdpTO
					.setOrganismoPdpId((null != organismoPdp.getOrganismoPdpId()) ? organismoPdp
							.getOrganismoPdpId().longValue() : null);
	 ...
	 */
	// //MIGRADO
	protected List<OrganismoPdpBean> getListOrganismoPdpBean(List<TblOrganismosPdp> lista)
			throws BusinessException {
		List<OrganismoPdpBean> result = null;
		SimpleDateFormat df = new SimpleDateFormat("dd/MM/yyyy");

		if (lista != null && !lista.isEmpty()) {
			result = new ArrayList<>();

			for (TblOrganismosPdp o : lista) {
				OrganismoPdpBean organismoPdp = new OrganismoPdpBean();
				...
				try {
					...
					organismoPdp.setOrganismoPdpId(o.getOrganismoPdpId().intValue());		
				} catch (IllegalAccessException | InvocationTargetException e) {
					throw new BusinessException(e);
				}
	 ...
	 */
	// /MIGRADO
	protected OrganismoPdpBean getOrganismoPdpBean(TblOrganismosPdp o)
			throws BusinessException {
		OrganismoPdpBean organismo = new OrganismoPdpBean();
		...
		try {
			...
			organismo.setOrganismoPdpId(o.getOrganismoPdpId().intValue());
		} catch (IllegalAccessException | InvocationTargetException e) {
	 ...
	 */
	private es.minhap.plataformamensajeria.iop.beans.OrganismoPdpBean createOrganismoPdpBean(
			OrganismoPdpBean criterio,
			OrganismoPdpBean ob) {
		try {
			if (null != criterio) {
				Date defaultValue = null;
