       ...
       public int doStartTag() throws JspException {
		...
		HashMap<Integer,Integer> mapPermisosAplicaciones =(HashMap<Integer, Integer>)pageContext.getSession().getAttribute(PlataformaMensajeriaUtil.MAP_PERMISOS_APLICACIONES); 
    	 ...
    	 if(rolUsuario!=null&&PlataformaMensajeriaUtil.ROL_PROPIETARIO.equals(rolUsuario)){
    	 	if(idAplicacion!=null&&!"".equals(idAplicacion)&&mapPermisosAplicaciones!=null){
	    		 Integer modoMap = mapPermisosAplicaciones.get(Integer.valueOf(idAplicacion));
	    		 if(modoMap!=null&&modoMap.intValue()==2){
	    			 skipBody=false;
	    		 }
	    	 }
    	 }else  if(rolUsuario!=null&&(PlataformaMensajeriaUtil.ROL_ADMINISTRADOR.equals(rolUsuario)||PlataformaMensajeriaUtil.ROL_CAID.equals(rolUsuario))){
    		 skipBody=false;
    	 }
         if(skipBody){
