...
<!-- ************************** -->
		<div class="mainContent">
	            ...
	            <label>Servidores Web Push</label>
          	</h3>
	...
	<%@include file="/WEB-INF/jsp/plataforma/validation/fieldErrorForm.jsp" %>	
            <div class="criteria">
		<s:form id="frmBuscarServidorWebPush" method="POST" action="buscarServidorWebPush"
		validate="false" theme="css_xhtml">            	
            	<p class="criteria">
                    <label class="fieldText" style="width: 100px;">Nombre:</label>
                    <s:textfield
							name="servidorWebPush.nombre" id="servidorWebPush.nombre"
							theme="simple" 
 		...
 		<s:set name="listaServidoresPush" value="%{listaServidoresPush}" />

		<s:form id="frmEliminarServidoresWebPushSeleccionados" method="POST" onsubmit="return confirmDeleteSelected();" action="deleteServidoresWebPushSeleccionados">
		<h4 class="titular">Encontradas <%=request.getAttribute("totalSize")%> entradas</h4>
		   <table class="tablaHeader">
			<thead>
               ...
               <script>
               	function checkBotonEliminarSeleccionados(){
               		var listaChecks = document.getElementById('frmEliminarServidoresWebPushSeleccionados').checkDelList;
			...
			<display:table 
				id="tableId"
				summary="Tabla de resultados de búsqueda de servidores Push"
				name="listaServidoresPush" 
				...
				<display:column class="darkTD TH15">
					...
					<input type="checkbox" onclick="checkBotonEliminarSeleccionados()" id="checkDelList" name="checkDelList" value="${tableId.servidorWebPushId }"/></center> 
					<input type="hidden" idd="__checkbox_checkDelList" name="__checkbox_checkDelList"/>
 				
				</display:column>
				<display:column property="servidorWebPushId" titleKey="field.servidorPush.id" sortable="false"
					headerClass="TH30 separator center" class="center" /> 
				<%-- nombre --%>
				<display:column property="nombre" titleKey="field.servidorPush.nombre" sortable="true"
					headerClass="TH170 separator center" class="" />
				
				<%-- descripcion --%>
				<display:column property="descripcion" titleKey="field.servidorPush.descripcion" sortable="false"
					headerClass="separator center" class="" />
				<display:column property="isDefecto" titleKey="field.servidorPush.porDefecto" sortable="false"
					headerClass="TH50 separator center" class="center" />					
				<display:column property="isActivo" titleKey="field.servidorPush.activo" sortable="false"
					headerClass="TH50 separator center" class="center" />
				<%-- acciones --%>
				<display:column property="servidorWebPushAction"  
				...
				<tr>
                    	<td colspan="5">
		                    ...
		                    <span class="rightSide">
                            	...
                            	<input  type="button" value="Nueva Entrada" class="button" onclick='javascript:location.href="nuevoServidorWebPush.action"'/>
                            </span>
                        </td>
                    </tr>
               </tfoot>
