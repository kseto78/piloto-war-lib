...
<div class="mainContent">
	<h3 class="pageNameButtons">
		<span class="floatRight"></span> <label>Usuarios</label>
	</h3>
	...
	<div class="criteria">
		<s:form id="frmBuscarUsuarios" method="POST" action="buscarUsuarios"
			...
			<p class="criteria">
				...
				<s:textfield name="usuario.nombre" id="usuario.nombre"
			...
			<p class="criteria">
				<span> <label style="width: 100px;" class="fieldText">Aplicacion:</label>
					<s:select cssStyle="width:200px;" id="usuario.aplicacionId"
						name="usuario.aplicacionId" emptyOption="false" theme="simple"
						labelposition="left" list="comboAplicaciones" listKey="codigo"
						listValue="descripcion" cssClass="" headerKey="-1"
						headerValue="Todos" value="%{usuario.aplicacionId}"
						disabled="false" />
				</span> <span> <label style="width: 100px;" class="fieldText">Rol:</label>
					<s:select cssStyle="width:150px;" id="usuario.rolId"
						name="usuario.rolId" emptyOption="false" theme="simple"
						labelposition="left" list="comboRoles" listKey="codigo"
						listValue="descripcion" cssClass="" headerKey="-1"
						headerValue="Todos" value="%{usuario.rolId}" disabled="false" />
				</span>
	...
	<s:form id="frmEliminarUsuariosSeleccionados" method="POST"
		onsubmit="return confirmDeleteSelected();"
		action="deleteUsuariosSeleccionados">
		<h4 class="titular">
			Encontradas
		...
		<script>
			function checkBotonEliminarSeleccionados() {
				var listaChecks = document
						.getElementById('frmEliminarUsuariosSeleccionados').checkDelList;
		...
		<display:table id="tableId"
			...
			name="listaUsuarios" class="" pagesize="${pageSize}" requestURI=""
			...
			<display:column class="darkTD TH15">
				<center>
					...
					<input type="checkbox" onclick="checkBotonEliminarSeleccionados()"
						...
						value="${tableId.usuarioId }" />
				</center>
				<input type="hidden" idd="__checkbox_checkDelList"
					name="__checkbox_checkDelList" />

			</display:column>
			<display:column property="usuarioId" titleKey="field.usuario.id"
				sortable="false" headerClass="TH30 separator center" class="center" />
			<%-- nombre usuario--%>
			<display:column property="nombre" titleKey="field.usuario.usuario"
				sortable="true" headerClass="separator center" class="" />

			<%-- nombre rol --%>
			<display:column property="rolUsuario"
				titleKey="field.usuario.rolUsuario" sortable="true"
		...
		<table>
			<tfoot>
				<tr>
					<td colspan="5"><span class="leftSide"> <!-- <input type="button" class="button" value="Eliminar Seleccionados">-->
							<s:submit id="eliminaSeleccionados" theme="simple"
								disabled="true"
							...
							onclick='javascript:location.href="nuevoUsuario.action"' />
					</span></td>
				</tr>
			</tfoot>
		</table>
