...
<!-- ************************** -->
		...
		<%@include file="/WEB-INF/jsp/plataforma/validation/fieldErrorForm.jsp" %>
            <div class="criteria">
<s:form id="frmBuscarServiciosMoviles" method="POST" action="buscarServiciosMoviles"
		validate="false" theme="css_xhtml">            	
            	<p class="criteria">
                    <label class="fieldText" style="width: 100px;">Nombre:</label>
                    <s:textfield
							name="servicioMovil.nombre" id="servicioMovil.nombre"
							theme="simple" 
 		...
 		<s:set name="listaServiciosMoviles" value="%{listaServiciosMoviles}" />

		<s:form id="frmEliminarServiciosMovilesSeleccionados" method="POST" onsubmit="return confirmDeleteSelected();" action="deleteServiciosMovilesSeleccionados">
		<h4 class="titular">Encontradas <%=request.getAttribute("totalSize")%> entradas</h4>
		   <table class="tablaHeader">
			<thead>
               ...
               <script>
               	function checkBotonEliminarSeleccionados(){
               		var listaChecks = document.getElementById('frmEliminarServiciosMovilesSeleccionados').checkDelList;
			...
			<display:table 
				id="tableId"
				summary="Tabla de resultados de búsqueda de servicios moviles"
				name="listaServiciosMoviles" 
				...
				<display:column class="darkTD TH15">
					...
					<input type="checkbox" onclick="checkBotonEliminarSeleccionados()" id="checkDelList" name="checkDelList" value="${tableId.servicioMovilId }"/></center> 
					<input type="hidden" idd="__checkbox_checkDelList" name="__checkbox_checkDelList"/>
 				
				</display:column>
				<%-- nombre --%>
				<display:column property="servicioMovilId" titleKey="field.servicioMovil.id" sortable="false"
					headerClass="TH30 separator center" class="center" />				
				<display:column property="nombre" titleKey="field.servicioMovil.nombre" sortable="true"
					headerClass="TH170 separator center" class="" />
				
				<%-- descripcion --%>
				<display:column property="descripcion" titleKey="field.servicioMovil.descripcion" sortable="false"
					headerClass="separator center" class="" />
				<display:column property="tipoServicio" titleKey="field.servicioMovil.tipo" sortable="true"
					headerClass="separator center" class="center" />
				<display:column property="isEstado" titleKey="field.servicioMovil.estado" sortable="false"
					headerClass="TH50 separator center" class="center" />
				<%-- acciones --%>
				<display:column property="servicioMovilAction" titleKey="field.servicioMovil.servicioMovilAction"
			...
			<tfoot>
				<tr>
                    	<td colspan="5">
		                    ...
		                    <span class="rightSide">
                            	<input  type="button" value="Nueva Entrada" class="button" onclick='javascript:location.href="nuevoServicioMovil.action"'/>
                            </span>
                        </td>
                    </tr>
               </tfoot>
