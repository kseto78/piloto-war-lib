...
<!-- ************************** -->
		...
		<sj:dialog  id="dialogTree" title="ARBOL ORGANISMO" cssStyle="display:none" autoOpen="false"></sj:dialog>
            <div class="criteria">
		<s:form id="frmBuscarOrganismosPdp" method="POST" action="buscarOrganismosPdp"
		...
		<s:form id="frmEliminarOrganismosSeleccionados" method="POST" onsubmit="return confirmDeleteSelected();" action="deleteOrganismosPdpSeleccionados">
		   ...
		   <table class="tablaHeader">
			...
			<display:table 
				...
				<display:column class="darkTD TH15" headerClass="TH15">
					...
					<input type="checkbox" onclick="checkBotonEliminarSeleccionados()" id="checkDelList" name="checkDelList" value="${tableId.organismoPdpId }"/></center> 
					<input type="hidden" idd="__checkbox_checkDelList" name="__checkbox_checkDelList"/>
 				
				</display:column>
				<%-- nombre --%>
				<display:column property="organismoPdpId" titleKey="field.organismopdp.id" sortable="false"
				...
				<display:column property="nombre" titleKey="field.organismopdp.nombre" sortable="true"
					headerClass="TH290 separator center" class="" />
				...
				<display:column property="organismoPdpAction"
					headerClass="TH10 separator center" class="" media="html" />
			</display:table>
			<table>
			<tfoot>
				<tr>
                    	<td colspan="5">
		                    ...
		                    <span class="rightSide">
                            	<input  type="button" value="Nueva Entrada" class="button" onclick='javascript:location.href="nuevoOrganismoPdp.action"'/>
                            </span>
                        </td>
                    </tr>
               </tfoot>
 ...
 function loadArbol(enlace,idOrganismo) {
		...
		var url = "arbolOrganismoPdp.action?idOrganismo=" + idOrganismo + "&arbol=true";
