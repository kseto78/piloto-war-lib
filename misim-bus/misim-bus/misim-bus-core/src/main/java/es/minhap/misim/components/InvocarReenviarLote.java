 ...
 * 
 */
public class InvocarReenviarLote implements Callable {
	
	private static final Logger LOG = LoggerFactory.getLogger(InvocarReenviarLote.class);
	...
	public Object onCall(final MuleEventContext eventContext) throws ModelException {
		...
		try{
			...
			NodeList peticion = docOriginal.getElementsByTagNameNS("http://misim.redsara.es/misim-bus-webapp/peticion", "PeticionLote");
			String xmlPeticion = XMLUtils.nodeToString(peticion.item(0));
			
			OperacionesLotesMensajesXMLBean operacionesMensajes = new OperacionesLotesMensajesXMLBean();
			operacionesMensajes.loadObjectFromXML(xmlPeticion);
			IOperacionesMensajesService operacionesService = FactoryServiceSim.getInstance().getInstanceOperaciones();
			String respuesta=operacionesService.reenviarLote(operacionesMensajes);
