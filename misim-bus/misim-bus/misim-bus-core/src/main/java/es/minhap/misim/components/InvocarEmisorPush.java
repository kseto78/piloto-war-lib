...
public class InvocarEmisorPush implements Callable {
	...
	private ResponseStatusType sendPushNotification(String idMensaje,  NotificacionDataRequest parametros, String keystore, String company) {
		ResponseStatusType response = new ResponseStatusType();
		try{
	...
	private NotificacionDataRequest leerParametros(Document docOriginal) throws Exception{
	 ...
	 private ResponseStatusType sendPUSH(String company, String keystore, NotificacionDataRequest notificacion, String idMensaje) throws ModelException, Exception{
		...
		try {    	
			...
			if("Google".equals(company)){
				...
				respuesta = sendToGoogle.enviarGoogle(notificacion.getgCMApiKey(), notificacion.getToken(), notificacion.getUrl(), notificacion.getBadge(),
						notificacion.getCabecera(), notificacion.getCuerpo(), notificacion.getSound(), notificacion.getIcon(), idMensaje);
				LOG.info("respuesta de google ->" + respuesta);
				Integer mensajesfallidos = Integer.parseInt(respuesta.substring(respuesta.indexOf("\"failure\":") + 10, respuesta.indexOf("\"canonical_ids\":") - 1));
				JSONObject jsonObject = new JSONObject(respuesta);
				String canonicalIds = (!jsonObject.isNull("canonical_ids")) ? jsonObject.getString("canonical_ids") : null;
				...
				if ((null != canonicalIds && "1".equals(canonicalIds)) || (null != results && results.contains("error"))) {
					respuesta = "ERROR,Token no valido";
					statusCode = KO_RESPONSE;
					IRegistroUsuarioPushService usuarioService = FactoryServiceSim.getInstance().getInstanceUsuario();
					usuarioService.eliminarUsuario(notificacion.getToken().get(0));
				} else {
					statusCode = OK_RESPONSE;
				}
				...
				//Probar a pasar directamente el keystore de sun sin necesidad de la libreria
				File file = new File(keystore+notificacion.getRutaCertificadoAPNS());
				PushNotificationSender pushSender = new PushNotificationSender(file, notificacion.getPasswordCertificadoAPNS(), produccion);
		        ...
		        if (notificacion.getCuerpo().contains("{")){
		        	...
		        	String title = (!jsonObject.isNull("title")) ? jsonObject.getString("title") : notificacion.getCabecera();
		        	String body = (!jsonObject.isNull("bodyMessage")) ? jsonObject.getString("bodyMessage") : "";
		        	
		        	String alert = "{\"aps\": {\"alert\":{\"title\":\"" + title + "\",\"body\":\"" + body + "\",\"action-loc-key\":\"PLAY\"}"
		        			+", \"badge\": \"" + badge
		        			+"\", \"sound\": \"" + sound
		        			+"\"},\"content\":" + data + "}";
		        	...
		        	String alert = "{\"aps\":{\"alert\":{\"title\":\""+notificacion.getCabecera()+"\", \"body\":\""+ notificacion.getCuerpo() +"\", \"action-loc-key\":\"PLAY\"}}}";
