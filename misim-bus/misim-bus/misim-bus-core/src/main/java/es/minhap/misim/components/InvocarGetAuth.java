 ...
 * 
 */
public class InvocarGetAuth implements Callable {

	private static final Logger LOG = LoggerFactory.getLogger(InvocarGetAuth.class);
	...
	public Object onCall(final MuleEventContext eventContext) throws ModelException {
		...
		try {
			IGetAuthService getAuth = FactoryServiceSim.getInstance().getInstanceGetAuth();
			...
			String respuesta = getAuth.getDatosUsuario(pet);
