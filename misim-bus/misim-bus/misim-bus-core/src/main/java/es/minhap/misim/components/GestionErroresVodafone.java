 ...
 * 
 */
public class GestionErroresVodafone implements Callable {

	private static final Logger LOG = LoggerFactory.getLogger(GestionErroresVodafone.class);
	...
	public Object onCall(final MuleEventContext eventContext) throws Exception {
		...
		response.setEstado(String.valueOf(codigoError));
		response.setCodigo(literalError);
		response.setDetalle(detailsError);

		
		SoapPayload.class.cast(eventContext.getMessage().getPayload()).setSoapMessage(XMLUtils.setPayloadFromObject(response, Charset.forName("UTF-8"), ResultadoOp.class));
