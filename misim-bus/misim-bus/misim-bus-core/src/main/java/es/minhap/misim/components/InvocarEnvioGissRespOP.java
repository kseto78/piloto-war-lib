 ...
 * 
 */
public class InvocarEnvioGissRespOP implements Callable {

	private static final Logger LOG = LoggerFactory.getLogger(InvocarEnvioGissRespOP.class);
...
//		Integer idServicioAEAT = new Integer(ps.getMessage("aeat.servicio.sms.premium", null, null, null));
		...
		try{
			IEnvioPremiumGISSService envioPremiumGISSService = FactoryServiceSim.getInstance().getInstanceGISS();
...
//			Peticion peticion = (Peticion) docOriginal.getElementsByTagNameNS("http://misim.redsara.es/misim-bus-webapp/peticion", "Peticion");
			NodeList respuesta = docOriginal.getElementsByTagNameNS("http://misim.redsara.es/misim-bus-webapp/respuesta", "Respuesta");
				...
				String respuestaOP=envioPremiumGISSService.cambiarEstadoSMSPremium(new Integer(respuestaEnvioXMLBean.getMessageId()), respuestaEnvioXMLBean.getStatus().getStatusText());
