 ...
 * 
 */
public class InvocarGestionNotificacionesPush implements Callable {
	
	private static final Logger LOG = LoggerFactory.getLogger(InvocarGestionNotificacionesPush.class);
	...
	public Object onCall(final MuleEventContext eventContext) throws ModelException {
		...
		try{
			...
			NodeList peticion = docOriginal.getElementsByTagNameNS("http://misim.redsara.es/misim-bus-webapp/peticionNotificacionPush", "PeticionNotificacionPush");
			String xmlPeticion = XMLUtils.nodeToString(peticion.item(0));
			NotificacionesPushXMLBean notificacionesPushXML = new NotificacionesPushXMLBean();
			notificacionesPushXML.loadObjectFromXML(xmlPeticion);

			IGestionNotificacionesPushService notificacionesPushService = FactoryServiceSim.getInstance().getInstanceNotificacionesPush();
			String respuesta=notificacionesPushService.notificacionCambioEstado(notificacionesPushXML);
