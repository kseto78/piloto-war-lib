...
public class RefreshStatusServiceImpl implements IRefreshStatusService, MuleContextAware {
	...
	public void refreshStatus(Long mensajeId, Long destinatarioMensajeId, Long loteId, String aplicacionPremium, String usuarioAplicacion) throws Exception{
		...
		if ("S".equals(ejecutarTodosServidores)) {
			LOG.info("[RefreshStatusServiceImpl] Se ejecutan todos los servicodores "+ ejecutarTodosServidores);
			...
			LOG.info("[RefreshStatusServiceImpl] Consultar primer proveedor.");
			sendOK = consultaPrimerProveedor(mensajeId, mapServicios, uim, smsData, loteId, aplicacionPremium);
		}
		LOG.info("[RefreshStatusServiceImpl] el parametro send es: "+((sendOK)?"true":"false"));
		if (!sendOK) {
			throw new Exception("[RefreshStatusServiceImpl.refreshStatus] -ERROR Actualizando Estado- Mensaje: " + mensajeId);
		}
		LOG.info("[RefreshStatusServiceImpl] el parametro repetirRefreshStatus es: "+((repetirRefreshStatus)?"true":"false"));
		if (repetirRefreshStatus){
			...
			if (null != smsData.destinatarioMensajeId){
				TblDestinatariosMensajes destinatariosMensaje = destinatariosMensajesManager.getDestinatarioMensaje(smsData.destinatarioMensajeId);
				
//				mns.setIdExterno(destinatariosMensaje.getCodigoexterno());
				mns.setDestinatarioMensajeId(smsData.destinatarioMensajeId.toString());
				
				//Comprobar el numero de reintentos
				numMaxReintentos = comprobarNumIntentosRefresh(ps, destinatariosMensaje, numMaxReintentos);
			}
//			else{
...
//					Comprobamos que si ya se ha actualizado la tabla de errores
	...
	/**
	 * Devuelve el numero de intentos para el proceso refreshStatus de ActiveMq
	 * 
	 * @param mensaje
	 * @param numMaxReintentos
	 * @return
	 */
	private Integer comprobarNumIntentosRefresh(PropertiesServices ps, TblDestinatariosMensajes destinatariosMensajes, Integer numMaxReintentos) {
		Integer result = numMaxReintentos;
		
		if (destinatariosMensajes != null) {
			
			if (destinatariosMensajes.getNumintentorefresh() != null && destinatariosMensajes.getNumintentorefresh() > 0) {
				destinatariosMensajes.setNumintentorefresh(destinatariosMensajes.getNumintentorefresh()-1);
				if (destinatariosMensajes.getNumintentorefresh() == 0) {
					String estadoAnulado = ps.getMessage("constantes.ESTADO_ANULADO", null);
					String descripcion = ps.getMessage("constantes.DESCRIPCION_ESTADO_ANULADO", null);
					destinatariosMensajes.setEstado(estadoAnulado);
					tblMensajesManager.setEstadoMensaje(destinatariosMensajes.getMensajeid(), estadoAnulado, descripcion, false, destinatariosMensajes.getDestinatariosmensajes(), null, null, null);
					LOG.debug("[RefreshStatusServiceImpl] Finaliza el proceso con estado, "+estadoAnulado+", y descripcion "+descripcion);
				}
			} else {
				LOG.debug("[RefreshStatusServiceImpl] Se marca por primera vez el numero maximo de intentos: "+numMaxReintentos);
				destinatariosMensajes.setNumintentorefresh(numMaxReintentos);
			}
			destinatariosMensajesManager.update(destinatariosMensajes);
			result = destinatariosMensajes.getNumintentorefresh();
		}
		LOG.debug("[RefreshStatusServiceImpl] Quedan unos "+result+" intentos.");
		return result;
	}
	...
	private boolean codificaRespuesta(long mensajeId, SMSData smsData, String respuesta, String aplicacionPremium, PropertiesServices pa) {
		boolean res = false;
		PropertiesServices ps = new PropertiesServices(reloadableResourceBundleMessageSource);
		LOG.info("[RefreshStatusServiceImpl] La respuesta de la operadora es:"+respuesta);
...
//			respuesta = "ERROR|-10|UIM not found";
			...
			if (exito.contains(ps.getMessage("plataformaErrores.recepcionEstado.STATUSTEXT_OK", null))) {
				cod = st.nextToken();
				...
				if (ultimoEstadoHistorialId != estadoMensaje.getEstadoId()){
					if(cod!=null && ps.getMessage("constantes.respuesta.rechazadoOperadora", null).equals(cod.trim())){				
				...
				cod = ps.getMessage("constantes.respuesta.errorIndeterminado", null);
			...
			if (ps.getMessage("constantes.respuesta.pedientePlataforma00", null).equals(cod) 
					|| ps.getMessage("constantes.respuesta.pedientePlataforma10", null).equals(cod) 
					|| ps.getMessage("constantes.respuesta.pedientePlataforma20", null).equals(cod) 
					|| ps.getMessage("constantes.respuesta.entregadoOperadora", null).equals(cod) ){
				repetirRefreshStatus = true;
			}
			LOG.info("[RefreshStatusServiceImpl.codificaRespuesta] el parametro repetirRefreshStatus es: "+((repetirRefreshStatus)?"true":"false"));
		} catch (Exception ex) {
		...
		try {
			...
			if (null != soapMessage) {
				...
				if (!respuestaAEAT.contains(ok)) {
					LOG.error(errorClave + "Error notificar estado AEAT Mensaje [" + mensajeId + "] : "+endpointUrl);
				}
			}
		} catch (Exception e) {
