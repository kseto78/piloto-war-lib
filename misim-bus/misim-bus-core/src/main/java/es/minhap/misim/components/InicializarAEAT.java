...
public class InicializarAEAT implements Callable {
	...
	private static final Logger LOG = LoggerFactory.getLogger(InicializarAEAT.class);
	...
	public Object onCall(final MuleEventContext eventContext) throws ModelException {
		...
		eventContext.getMessage().setOutboundProperty("peticionAEAT", "true");
		...
		try {
			...
			if (listaProveedores == null || listaProveedores.isEmpty()) {
				...
				if (listaProveedores.get(0).getEndpoint() != null) {
					...
					eventContext.getMessage().setOutboundProperty("endpointUrl", getUrlEndpoint(eventContext));
			...
			LOG.error("Identificación: Error de sistema al inicializar AEAT", e);
			throw new ModelException(e.getMensaje(), e.getCodigo());

		} catch (Exception e) {

			LOG.error("Identificación: Error de sistema al inicializar AEAT", e);
			throw new ModelException("Error de sistema al inicializar AEAT", 502);
		}
