...
public class InvocarProcesarSAML implements Callable {
	...
	@Resource
	IGestionSAMLResponseService gestionSAMLResponseImpl;
	...
	public Object onCall(final MuleEventContext eventContext) throws ModelException {
		...
		try {
			...
			NodeList peticion = docOriginal.getElementsByTagNameNS(
					"http://misim.redsara.es/misim-bus-webapp/PeticionClaveAuthResponse", "PeticionClaveAuthResponse");
			...
			datosPersonales = ClaveAuthenticationFilter.attemptAuthentication(ps, peticionResponse.getSamlResponse(),
					peticionResponse.getRemoteHost());
			...
			respuesta = gestionSAMLResponseImpl.insertarDatosUsuario(peticionResponse, nombre, NIF, apellido1,
					apellido2);
