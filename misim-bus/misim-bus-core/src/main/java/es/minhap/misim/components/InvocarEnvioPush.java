 ...
 * 
 */
public class InvocarEnvioPush implements Callable {
	
	private static final Logger LOG = LoggerFactory.getLogger(InvocarEnvioPush.class);
...
//			envioPUSH.loadObjectFromXML(xmlPeticion);
//			IEnvioMensajesService envioService = FactoryServiceSim.getInstance().getInstanceMensajes();
//			String respuesta=envioService.enviarNotificacion(envioPUSH);
//			SOAPMessage responseMessage=getSoapMessageFromString(respuesta);
			...
			String respuesta=envioService.enviarLotesNotificacion(peticionXML, ps);
