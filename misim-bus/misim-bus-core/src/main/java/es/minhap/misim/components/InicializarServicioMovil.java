...
public class InicializarServicioMovil implements Callable {

	private static final Logger LOG = LoggerFactory.getLogger(InicializarServicioMovil.class);

	private static final String PRODUCTO_AVISO = "SUSCRIPCION_USUARIO_PUSH";
	private static final String PROVEEDOR = "Pruebas";
	...
	public Object onCall(final MuleEventContext eventContext) throws ModelException {
		...
		eventContext.getMessage().setOutboundProperty("errorServicioMovil", "false");
		eventContext.getMessage().setOutboundProperty("peticionServicioMovil", "true");
		...
		try {
			...
			if ((PRODUCTO_AVISO).equals(soapAction)) {
				arg1.setCodigo(PRODUCTO_AVISO);
			} else {
			...
			if (listaProveedores == null || listaProveedores.isEmpty()) {
				...
				if (listaProveedores.get(0).getCertificado() != null) {
					eventContext.getMessage().setOutboundProperty("certificado",
							listaProveedores.get(0).getCertificado());
				}
				if (listaProveedores.get(0).getCertificadoPass() != null) {
					eventContext.getMessage().setOutboundProperty("certificadoPass",
							listaProveedores.get(0).getCertificadoPass());
				}
				...
				if (listaProveedores.get(0).getNodoCifrado() != null) {
					eventContext.getMessage().setOutboundProperty("nodoCifrado",
							listaProveedores.get(0).getNodoCifrado());
				}
				if (listaProveedores.get(0).getEsquemaCifrado() != null) {
					eventContext.getMessage().setOutboundProperty("esquemaCifrado",
							listaProveedores.get(0).getEsquemaCifrado());
				}

				// ENDPOINT
				if (listaProveedores.get(0).getEndpoint() != null) {

					if(eventContext.getMessage().getOutboundProperty("endpointUrl") == null && listaProveedores.get(0).getEndpoint().getUrlEndpoint()!=null){
					...
					if (listaProveedores.get(0).getEndpoint().getComunicacion() != null
						...
						eventContext.getMessage().setOutboundProperty("idComunicacion",
								listaProveedores.get(0).getEndpoint().getComunicacion().getIdComunicacion());
					}
				}
				...
				if (listaProveedores.get(0).getTransformacion() != null) {
					...
					if (listaProveedores.get(0).getTransformacion().getXslPeticion() != null) {
						...
						eventContext.getMessage().setOutboundProperty("xslPeticion",
								listaProveedores.get(0).getTransformacion().getXslPeticion());
					} else {
					...
					if (listaProveedores.get(0).getTransformacion().getXslRespuesta() != null) {
						...
						eventContext.getMessage().setOutboundProperty("xslRespuesta",
								listaProveedores.get(0).getTransformacion().getXslRespuesta());
					} else {
					...
					if (listaProveedores.get(0).getTransformacion().getXslFault() != null) {
						eventContext.getMessage().setOutboundProperty("xslSoapFault",
								listaProveedores.get(0).getTransformacion().getXslFault());
					}
				} else {
					eventContext.getMessage().setOutboundProperty("transformacionPeticion", "N");
					eventContext.getMessage().setOutboundProperty("transformacionRespuesta", "N");
			...
			LOG.error("Identificación: Error de sistema al inicializar Servicio Movil", e);
			throw new ModelException(e.getMensaje(), e.getCodigo());

		} catch (Exception e) {

			LOG.error("Identificación: Error de sistema al inicializar Servicio Movil", e);
			throw new ModelException("Error de sistema al inicializar Servicio Movil", 502);
		}
