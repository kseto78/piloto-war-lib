/*
 *
 * archivo: TblLotesEnviosQuery.java
 *
...
/**
 * Clase con criterios de busqueda para la entidad TblLotesEnvios
 */
public class TblLotesEnviosQuery extends
		AbstractHibernateQueryEntity<TblLotesEnvios> {
	 ...
	 */
	public TblLotesEnviosQuery(Long loteenvioid) {
	 ...
	 * Permite buscar cuando campo multidestinatario es NOT NULL
	 * 
	 * @param multidestinatarioIsNotNull
	 *            Valor de seteo.
	 */
	...
	public void addCriteria(Criteria criteria, boolean useOrder) {
		...
		if (isInnerJoinTblServicios()) {
			tblServiciosCriteria = criteria.createCriteria(TBLSERVICIOS, "a_"
					+ TBLSERVICIOS, CriteriaSpecification.INNER_JOIN);
		} else if (isLeftJoinTblServicios()) {
			tblServiciosCriteria = criteria.createCriteria(TBLSERVICIOS, "a_"
					+ TBLSERVICIOS, CriteriaSpecification.LEFT_JOIN);
		}

		if (getTblServicios() != null) {
			if (getTblServicios().getServicioid() == null) {
				if (tblServiciosCriteria == null) {
					tblServiciosCriteria = criteria.createCriteria(
							TBLSERVICIOS, "a_" + TBLSERVICIOS);
				}
		...
		if (getDescripcion() != null) {
			if (getDescripcionComparator() == TextComparator.EQUALS) {
				...
				criteria.add(Restrictions
						.sqlRestriction(createUpperTranslateSQL(DESCRIPCION,
								getDescripcion())));
			} else {
		...
		if (getMultidestinatario() != null) {
			criteria.add(Restrictions.eq(MULTIDESTINATARIO,
