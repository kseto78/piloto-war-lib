/*
 * archivo: ViewServidoresServiciosDAO.java
 *
 * Proyecto: Administracion SIM
 *
 * Generated file! Do not modify.
 *
 * Developed by:
 *     everis S.A.
 *     www.everis.com
 */

package es.minhap.sim.dao;

import es.minhap.common.dao.EntityDAO;
import es.minhap.sim.model.ViewServidoresServicios;
import es.minhap.sim.model.ViewServidoresServiciosId;
import es.minhap.sim.query.ViewServidoresServiciosQuery;

/**
 * Interfaz DAO que extiende de interfaz generica
 */
public interface ViewServidoresServiciosDAO
		extends
		EntityDAO<ViewServidoresServicios, ViewServidoresServiciosId, ViewServidoresServiciosQuery> {

}